<?php
    ...
    define('BASE_PATH', dirname(PUBLIC_PATH) ); // just used to compose CORE_PATH & MODULES_PATH
    ...
    // Start capturing 'level 0' output buffer stack
    ob_start();
     
    // SESION START/RESTART
    if (!session_start()) FG_error('Imposible iniciar sesión en front_controller...');
    ...
    // Control if user ir properly loged in
    ...
    function FG_render ($template = 'template_default' ) {
        ...
        {
            FG_error ('La plantilla requerida (' . $template . ') no existe...');
        }
        flush();
        exit;
    }
